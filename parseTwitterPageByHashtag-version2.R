...
getData <- function(hashtag, inputFileName, outputFileName) {
  ...
  # get reply, retweet fav count
  tweets.interaction <- xpathSApply(doc, "//span[@class='ProfileTweet-actionCountForAria']", xmlValue)
  tweets.reply <- vector()
  tweets.retweet <- vector()
  tweets.fav <- vector()
  
  count = 1
  i = 1
  while (count <= length(tweets.interaction)) {
    tweets.reply[i] = tweets.interaction[count]
    tweets.retweet[i] = tweets.interaction[count+1]
    tweets.fav[i] = tweets.interaction[count+2]
    count = count + 3
    i = i + 1
  }
  
  # merge the data into a dataframe
  data <- data.frame(name.screen, tweets.messyContent, tweets.content, tweets.hasPicture, tweets.hasSuperLink, tweets.count, tweets.time, tweets.reply, tweets.retweet, tweets.fav)
